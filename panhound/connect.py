"""Native cognis-connect emit for panhound — forward findings to any platform.

Maps panhound's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    panhound ... --format json | panhound-emit --to stix
    panhound-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a panhound record toward the Finding contract before normalize().

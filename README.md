...
[![PyPI](https://img.shields.io/pypi/v/cognis-panhound.svg)](https://pypi.org/project/cognis-panhound/)
[![CI](https://github.com/cognis-digital/panhound/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/panhound/actions)
...
PANHOUND exists for one job — scans code, logs, fixtures, and s3 buckets for leaked pans (luhn-validated card numbers) and cvvs before they hit prod. — and does it without a SaaS bill or heavyweight setup.
...
```bash
panhound --version
panhound scan .                      # scan the current project
panhound scan . --format json
panhound scan . --fail-on high       # non-zero exit for CI gates
panhound mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **gitleaks / trufflehog**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`panhound` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
